...
const vertifyJWTToken = (jwtToken, publicKey) => {
    return new Promise((resolve, reject) => {
        jwt.verify(jwtToken, publicKey, { algorithms: ['RS256'] }, (err, decoded) => {
            if (err) {
                reject(err);
            } else {
                resolve(decoded);
...
exports.handler = (event, context, callback) => {
    ...
    return rp(`https://${Auth0ApiBaseUrl}/.well-known/jwks.json`).then((jwks) => {
        ...
        if (jwks.alg !== 'RS256' || decodedToken.header.alg !== 'RS256') {
        ...
        return cert;
    }).then((publicKey) => vertifyJWTToken(jwtToken, publicKey))
    .then(() => {
